const form = {
  validParams: function (that, params) { //校验必填信息 多形态   添加进件
    ...
    arr.map(function (item) {
      if (!params[item] &&item!='custType'&&item!='specialBusiness'&&item != 'otherPhoto1' && item != "otherPhoto2" && item != "custId" && item != 'userId' && item != 'roleId' && item != 'custId' && item != 'licenceForOpeningAccounts' && item != 'bankCardFront') {
        console.log(item);
